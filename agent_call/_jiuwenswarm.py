"""Jiuwenswarm agent — pexpect-based TUI automation for ``jiuwenswarm-tui``."""
...
_LOG_NAME = "jiuwenswarm_chat_log"
...
def _get_module_logger() -> logging.Logger:
    ...
    if not log.handlers:
        ...
        path = os.path.join(os.getcwd(), "jiuwenswarm_chat.log")
...
def _post_process_interaction_log(log_file_path: str | os.PathLike[str]) -> str:
    ...
    # Remove specific known patterns
    remove_pattern = [
    ...
    # Strip remaining SGR color codes (e.g. \x1b[38;2;R;G;Bm, \x1b[48;2;R;G;Bm)
    log_content = re.sub(r"\x1b\[38;2;\d+;\d+;\d+m", "", log_content)
    log_content = re.sub(r"\x1b\[48;2;\d+;\d+;\d+m", "", log_content)
    log_content = re.sub(r"\x1b\[49m", "", log_content)
    # Strip cursor control sequences
    log_content = re.sub(r"\x1b\[\?25[lh]", "", log_content)
    log_content = re.sub(r"\x1b\[\?2004[lh]", "", log_content)
    log_content = re.sub(r"\x1b\[\?u", "", log_content)
    log_content = re.sub(r"\x1b\[\?2026[lh]", "", log_content)
    log_content = re.sub(r"\x1b\[\?1000[lh]", "", log_content)
    log_content = re.sub(r"\x1b\[\?1006[lh]", "", log_content)
    # Strip cursor movement
    log_content = re.sub(r"\x1b\[\d+[ABCDG]", "", log_content)
    # Strip other SGR attributes
    log_content = re.sub(r"\x1b\[[0-9;]*m", "", log_content)
    # Strip OSC (title, hyperlink) sequences
    log_content = re.sub(r"\x1b\][^\x07]*\x07", "", log_content)
...
def _interact_with_jiuwenswarm(
    input_string: str,
    *,
    interaction_log_file: str | os.PathLike[str] = "jiuwenswarm_interaction.log",
    cwd: str | os.PathLike[str] | None = None,
    timeout: float = 600,
    tee: bool = True,
    session_id: str | None = None,
    command: str = "jiuwenswarm-tui",
    reset_workspace: bool = False,
) -> Optional[str]:
    """Low-level pexpect automation of ``jiuwenswarm-tui``.
    ...
    try:
        ...
        _m_logger.info("等待2秒后按回车（确认安全检查和workspace）")
        _just_wait(child, 2)
        child.sendline("")
...
class JiuwenswarmAgent(BaseAgent):
    """Agent that automates ``jiuwenswarm-tui`` via pexpect."""

    name = "jiuwenswarm"
    output_filename_template = "{chat_id}_jiuwenswarm_output.txt"
    ...
    def execute(
        ...
        **kwargs,
        ...
        result = _interact_with_jiuwenswarm(
            prompt,
